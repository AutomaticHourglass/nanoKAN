...
class KAN(nn.Module):
    ...
    def __init__(self, config:KANConfig):
        ...
        for l in range(self.depth):
            ...
            # bias
            bias = nn.Linear(config.width[l + 1], 1, bias=False).requires_grad_(config.bias_trainable)
            bias.weight.data *= 0.
        ...
        self.k = config.k
        self.base_fun = config.base_fun

        ### initializing the symbolic front ###
        self.symbolic_fun = []
        for l in range(self.depth):
            sb_batch = Symbolic_KANLayer(in_dim=config.width[l], out_dim=config.width[l + 1])
            self.symbolic_fun.append(sb_batch)

        self.symbolic_fun = nn.ModuleList(self.symbolic_fun)
        self.symbolic_enabled = config.symbolic_enabled

        # Embedding Related parts
        self.device = config.device
        self.vocab_size = config.vocab_size
        self.block_size = config.block_size
        self.te = torch.nn.Embedding(self.vocab_size, self.width[0], device=config.device)
        self.pe = torch.nn.Embedding(self.block_size, self.width[0], device=config.device)

        self.lm_head = nn.Linear(self.width[-1], self.vocab_size, bias=False).to(config.device)
    ...
    def train2(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0.,
